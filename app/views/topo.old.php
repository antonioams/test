...
$matual = $db->query(" select * from modulo where upper(link)=upper('{$modulo}')");
    ...
    $m = $db->query("
        ...
        m.cdmodulopai is null and m.cdmodulo not in (1)
    ...
    $con = $db->query(" select * from consulta ");
...
<img src="/'.PROJETO.'/inc/img/faceless.jpg" class="header-avatar img-circle" alt="user" title="user">
                    ...
                    <ul class="dropdown-menu animated fadeInLeft">
                        <li>
                            <a href="/'.PROJETO.'/modulos/">Configurações</a>
                        </li>
                        <li>
                        ...
                        <li class="divider"></li>
                          <li>
                            <a href="/'.PROJETO.'/index/sair/">Sair</a>
                        </li>
                    </ul>
                </li>
            </ul>
...
echo '    <section class="layout">
    ...
    foreach ($am as $key ) {
        ...
        if ($amatual[0]['cdmodulo']==$key['cdmodulo']) {
           $act = ' class="active"';
                               $open = 'class="open"';
        ...
        m.cdmodulo=mc.cdmodulo and mc.cdperfil=".$_SESSION[PROJETO]['cdperfil']." and
        m.cdmodulo not in (select cdvinculado from modulo_vinculado) and
        m.cdmodulopai={$cdm} and m.cdmodulo not in (1)
        group by m.cdmodulo, m.nome, m.link, m.tipoimg,
                 m.atalho, m.restrito, m.cdmodulopai, m.ordem
        order by m.ordem, m.cdmodulo
                 ");
...
$m3 = $db->query("
        ...
        m.cdmodulo=mc.cdmodulo and mc.cdperfil=".$_SESSION[PROJETO]['cdperfil']." and
        m.cdmodulo not in (select cdvinculado from modulo_vinculado) and
        m.cdmodulopai={$cdm2} and m.cdmodulo not in (1)
        group by m.cdmodulo, m.nome, m.link, m.tipoimg,
                 m.atalho, m.restrito, m.cdmodulopai, m.ordem
        order by m.ordem, m.cdmodulo
                 ");
    ...
    foreach ($am as $key ) {
        ...
        if ($key['qtde']>0) {
            ...
            $m2 = $db->query("
                ...
                where
                m.cdmodulo=mc.cdmodulo and mc.cdperfil=".$_SESSION[PROJETO]['cdperfil']." and
                m.cdmodulopai={$cdm} and m.cdmodulo not in (select cdvinculado from modulo_vinculado)
                order by m.ordem, m.cdmodulo
                 ");
