...
<form method="POST" name="meuFormulario" action="/<?php echo PROJETO;?>/grupodocumento/atualizaj/id/<?php echo $view_grupodocumento[0]['cdgrupodocumento'];?>">

<div class="form-group">
<label  class="control-label">Descrição</label>
<div class="controls">
<div class="row"><div class="col-xs-12"><input id="descricao" name="descricao" type="text" value="<?php echo ( empty( $_POST ) ) ? ( $view_grupodocumento[0]['descricao'] ) : ( $_POST['descricao'] ) ?>" class="form-control"  data-parsley-trigger="change" placeholder="Descrição"/></div></div>
</div></div>
...
<div class="controls">
<div class="row"><div class="col-xs-12"><input id="tipo" name="tipo" type="text" value="<?php echo ( empty( $_POST ) ) ? ( $view_grupodocumento[0]['tipo'] ) : ( $_POST['tipo'] ) ?>" class="form-control"  data-parsley-trigger="change" placeholder="Tipo"/></div></div>
</div></div>

<div class="form-group" <?php echo ( ($view_vc[0]['chave'] == 'indicador') ? "style='display:none'" : "" )?>>
<label  class="control-label">Indicador</label>
<div class="controls">
<div class="row"><div class="col-xs-12"><input id="indicador" name="indicador" type="text" value="<?php echo ( empty( $_POST ) ) ? ( $view_grupodocumento[0]['indicador'] ) : ( $_POST['indicador'] ) ?>" class="form-control"  data-parsley-trigger="change" placeholder="Indicador"/></div></div>
</div></div>

<div class="form-group" <?php echo ( ($view_vc[0]['chave'] == 'formula') ? "style='display:none'" : "" )?>>
<label  class="control-label">Fórmula</label>
<div class="controls">
<div class="row"><div class="col-xs-12"><input id="formula" name="formula" type="text" value="<?php echo ( empty( $_POST ) ) ? ( $view_grupodocumento[0]['formula'] ) : ( $_POST['formula'] ) ?>" class="form-control"  data-parsley-trigger="change" placeholder="Fórmula"/></div></div>
</div></div>
...
<textarea class="form-control" name="fonte" rows="4"><?php echo ( empty( $_POST ) ) ? ( $view_grupodocumento[0]['fonte'] ) : ( $_POST['fonte'] ) ?></textarea>
</div></div>
</div></div>
...
<div class="controls">
<div class="row"><div class="col-xs-12">
<textarea class="form-control" name="descricao_indicador" rows="4"><?php echo ( empty( $_POST ) ) ? ( $view_grupodocumento[0]['descricao_indicador'] ) : ( $_POST['descricao_indicador'] ) ?></textarea>
</div></div>
</div></div>
...
<div class="control-group" id="positionGroup">
     <div class="controls icheck">
         <h5>Ativo</h5>
       <div class="mb5 mt5">
           <input type="checkbox" id="minimal-checkbox-1" name="ativo" value="1" <?php if ($view_grupodocumento[0]['ativo']==1) { echo 'checked';} ?>>
       </div>
      </div>
</div>
...
<a href="#"  class="btn btn-danger btn-sig" onclick="javascript: if (confirm('Deseja realmente excluir o Checklist? Todos os cadastros vinculados a ele tambem serao excluidos.'))location.href='/<?php echo PROJETO;?>/grupodocumento/exclui/id/<?php echo $view_grupodocumento[0]['cdgrupodocumento'];?>'">
