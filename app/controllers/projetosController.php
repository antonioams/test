        ...
        public function Index_action(){
           ...
           //$projetos_lista = $projetos->listaprojetos("{$par}");
            ...
            if(in_array($a,$hist)){
                        ...
                        $b= $aux[0]['cdhistorico'];
                        $conan = $projetos->rsql("SELECT p.*, h.texto, a.nome AS area, a.icone AS icone,  m.nome AS municipio_nome, f.descricao AS fase, t.nome AS tipo FROM projeto p 
                                                 LEFT JOIN tipo t ON p.cdtipo = t.cdtipo
                                                  ...
                                                  LEFT JOIN situacao f ON p.cdsituacao = f.cdsituacao
                                                  WHERE p.cdprojeto = ".$a."
                                                  AND h.cdhistorico = ".$b."
             ...
             //   print_r($a); echo "<br>"; 
                 $conan = $projetos->rsql("SELECT p.*, a.nome AS area, a.icone AS icone,  m.nome AS municipio_nome, f.descricao AS fase ,t.nome AS tipo FROM projeto p
                                                LEFT JOIN tipo t ON p.cdtipo = t.cdtipo 
                                                  LEFT JOIN area a ON p.cdarea = a.cdarea
                                                  LEFT JOIN municipio m ON p.cdmunicipio = m.cdmunicipio
                                                  LEFT JOIN situacao f ON p.cdsituacao = f.cdsituacao
     ...
     /* print('qtdefases: '.$qtdefases.'<br>');
      ...
      $datas['check'] = 0;      
        ...
        public function atualiza(){
            ...
            $historico_lista = $projetos->rsql("select p.flicker from projeto p, historico h where p.cdprojeto=h.cdprojeto and h.cdprojeto=".$id." and h.tabela='Fotos Flickr'");            
            
            if ( ($historico_lista[0]['flicker']!='') and ($historico_lista[0]['flicker']!=$dados['flicker']) ) {
               $atualiza = $historico->excluihistorico("cdprojeto=".$id." and tabela='Fotos Flickr'");
            }
        ...
        public function inserew(){
           ...
           $projeto_produto = new projeto_produtoModel();
            ...
            //historico    
            $produtoscon = $projetos->rsql("SELECT * FROM projeto_produto pp  
                                        JOIN produto p ON pp.cdproduto = p.cdproduto
                                        WHERE pp.cdprojeto = ".$inserep);   
            $textoproduto = '<br/><b>Produtos: </b>';
            foreach ($produtoscon as $prokey) {
                 $textoproduto .= $prokey['logica']."<br/>";
             } 
            

            $consulta = $projetos->listaprojetos('cdprojeto='.$inserep);
            $texto = '<b>Intervenção: </b>'.$consulta[0]['intervencao'].'<br/> <b> Objetivo: </b>'.$consulta[0]['objetivo'].'<br/> <b> Endereço: </b>'.$consulta[0]['endereco'].$textoproduto;
            $historico = new historicoModel();
            $dataHora = date("d/m/Y h:i:s");
            $dados_historico['tabela']='projeto';
