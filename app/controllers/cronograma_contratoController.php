<?php
session_start();
    class cronograma_contrato extends Controller {

        public function Index_action(){
        $cronograma_contrato = new cronograma_contratoModel();  
			  $projetos = new projetosModel();
		    $datas['vc'] = $this->modvinculados(get_class($this));	
			  
        $mes_lista =  $projetos->rsql("SELECT extract(month from min(data)) as mesi, extract(year from min(data)) as anoi  from contrato_item where cdcontrato in (select cdcontrato from contrato where cdcontrato=".$_SESSION[PROJETO]['filtro']['cdcontrato'].')');
        ...
        SELECT c.cdcontrato, c.numero, c.objeto,  ci.cdcontrato_item as codigo, 1 as tipo ,ci.cdcontrato_item, ci.descricao, ci.data, ci.cdcontrato_item_sup, ci.subitem, extract(month from ci.data) as mes, extract(year from ci.data) as ano, max(cp.sequencia) as sequencia, sum(cp.quantidade) as previsao_quantidade, sum(cp.valor) as previsao_valor
         ...
         where ci.cdcontrato_item_sup is null and c.cdcontrato=ci.cdcontrato and c.cdcontrato=".$_SESSION[PROJETO]['filtro']['cdcontrato']."
         ...
         where ci.cdcontrato_item_sup is not null and c.cdcontrato=ci.cdcontrato and c.cdcontrato=".$_SESSION[PROJETO]['filtro']['cdcontrato']."
         group by c.cdcontrato, c.numero, c.objeto,  ci.cdcontrato_item, ci.descricao, ci.data, ci.cdcontrato_item_sup, ci.subitem
        ...
        $datas['medicao_total'] = $medicao_total_lista; 
            ...
            if ($cronograma_contrato->pacesso('visualizar',$_SESSION[PROJETO]['cdperfil'])=='1') {
